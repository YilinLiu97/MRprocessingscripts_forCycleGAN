    ...
    for i, slice in enumerate(vol[1]):
        ...
        if istrain:
           ...
           for k in scaling_factors:
               slice  = clipped_zoom(slice_1,k)
               imsave('{}/{}slice_scaledBy_{}_{}.jpg'.format(foldername,ID,k,num),np.array(slice))
               num = num+1

               print slice.shape
               print '...The slice was successfully saved as {}slice_offlineScaledBy_{}_{}.jpg'.format(ID,k,num)

           for k in rotation_angles:
               slice  = imrotate(slice_1,k)
               imsave('{}/{}slice_rotatedBy_{}_{}.jpg'.format(foldername,ID,k,num),np.array(slice))
               num = num+1

               print slice.shape
               print '...The slice was successfully saved as {}slice_offlineRotatedBy_{}_{}.jpg'.format(ID,k,num)


        else:
           slice_1 = imrotate(slice_1,90)
#           slice_1 = imresize(slice_1,[256,256])
...
foldername = 'testB'   #the folder to save the slices
istrain = False
#########################################################
